    ...
    public class FlexalonFlexibleLayout : LayoutBase
    {
        ...
        private void CreateLines(FlexalonNode node, int flexAxis, int wrapAxis, int thirdAxis, bool wrap, Vector3 size, bool measure)
        {
            ...
            foreach (var child in node.Children)
            {
                ...
                var childSize = measure ? child.GetMeasureSize() : child.GetArrangeSize();
        ...
        private Vector3 MeasureTotalLineSize(bool wrap, int flexAxis, int wrapAxis, int thirdAxis)
        {
            ...
            foreach (var line in _lines)
            {
                ...
                else
                {
                    ...
                    FlexalonLog.Log("MeasureTotalLineSize", _node, line.Size);
                }
            }
        ...
        private bool UpdatePercentChildSizes(Vector3 size, int flexAxis, int wrapAxis, int thirdAxis)
        {
            ...
            float wrapRemainingSpace = size[wrapAxis];
            if (_wrapGapType == GapOptions.Fixed)
            {
                wrapRemainingSpace -= _wrapGap * (_lines.Count - 1);
            }

            wrapRemainingSpace = Mathf.Max(0, wrapRemainingSpace);

            foreach (var line in _lines)
            {
                line.Fill = true;
                float remainingSpace = size[flexAxis] - line.Size[flexAxis];
                remainingSpace -= 1e-6f; // Avoid floating point error causing a wrap.
                remainingSpace = Mathf.Max(0, remainingSpace);
