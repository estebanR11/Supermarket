...
namespace Flexalon
{
    ...
    public class FlexalonCircleLayout : LayoutBase
    {
        ...
        public override Bounds Measure(FlexalonNode node, Vector3 size)
        {
            ...
            {
                size[circleAxis1] = diameter;
            }

            if (node.GetSizeType(circleAxis2) == SizeType.Layout)
            {
                size[circleAxis2] = diameter;
            }
            ...
            if (_spiral)
            {
                ...
                foreach (var child in node.Children)
                {
                    ...
                    float childHeight = child.GetMeasureSize()[heightAxis];
            ...
            if (node.GetSizeType(heightAxis) == SizeType.Layout)
            {
                if (_spiral)
                {
                    size[heightAxis] = spiralHeight;
                }
                else
                {
                    foreach (var child in node.Children)
                    {
                        size[heightAxis] = Mathf.Max(size[heightAxis], child.GetMeasureSize()[heightAxis]);
                    }
                }
            }
            ...
            foreach (var child in node.Children)
            {
                if (child.GetSizeType(heightAxis) == SizeType.Fill)
                {
                    percentTotal += child.SizeOfParent[heightAxis];
                }
            }

            float remainingHeight = Mathf.Max(0, size[heightAxis] - spiralHeight);

            var childAvailableCircle = (node.Children.Count <= 2 && _spacingType == SpacingOptions.Evenly) ? 1 :
                diameter * Mathf.Tan(spacing / 2);

            foreach (var child in node.Children)
            {
                float childAvailableHeight = size[heightAxis];
                if (_spiral)
                {
                    var percent = percentTotal <= 1 ?
                        child.SizeOfParent[heightAxis] :
                        (child.SizeOfParent[heightAxis] / percentTotal);
                    childAvailableHeight = percent * remainingHeight;
                }

                var childAvailableSize = Vector3.zero;
                childAvailableSize[circleAxis1] = childAvailableCircle;
                childAvailableSize[circleAxis2] = childAvailableCircle;
                childAvailableSize[heightAxis] = childAvailableHeight;
                child.SetFillSize(childAvailableSize);
            }
